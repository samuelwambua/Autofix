...
import { Calendar, Plus, Search, Eye, X, Star, Shield } from 'lucide-react';
...
const fetchMyAppointments  = () => axiosInstance.get('/appointments/my-appointments').then(r => r.data);
const fetchMyVehicles      = () => axiosInstance.get('/vehicles/my-vehicles').then(r => r.data);
const fetchSupervisors     = () => axiosInstance.get('/appointments/supervisors').then(r => r.data);
...
const MyAppointments = () => {
  ...
  const { data: supervisorsData }    = useQuery({ queryKey: ['activeSupervisors'], queryFn: fetchSupervisors });

  const bookMutation = useMutation({
    mutationFn: (data) => axiosInstance.post('/appointments', data).then(r => r.data),
    onSuccess: () => {
      toast.success('Appointment booked successfully.');
      queryClient.invalidateQueries(['myAppointments', 'clientDashboard']);
      setShowBook(false); reset(); setSelectedSupervisor(null);
    },
  ...
  const onSubmit = (data) => {
    // Clean empty supervisor_id
    if (!data.supervisor_id) delete data.supervisor_id;
    ...
    <PageWrapper title="My Appointments" subtitle="Book and manage your service appointments.">
      ...
      <GlassCard className="p-4 mb-6">
        <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
          ...
          <button onClick={() => { setShowBook(true); reset(); setSelectedSupervisor(null); }}
              ...
              <GlassCard key={a.id} className="p-4 hover:bg-white/15 transition-all">
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {a.mechanic_name && (
                      <p className="text-white/25 text-xs">Mechanic: {a.mechanic_name}</p>
                    )}
                  </div>
      ...
      <Modal isOpen={showBook} onClose={() => { setShowBook(false); reset(); setSelectedSupervisor(null); }}
        title="Book Appointment" size="md">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="flex flex-col gap-2">
            <label className="text-white/70 text-sm font-medium">
              Choose Supervisor <span className="text-white/30 font-normal">(optional)</span>
            </label>
            <input type="hidden" {...register('supervisor_id')} />
            {supervisors.length === 0 ? (
              <p className="text-white/30 text-xs">No supervisors available.</p>
            ) : (
                  ...
                  return (
                    <button key={sup.id} type="button"
                      ...
                      <div className={`w-9 h-9 rounded-xl flex items-center justify-center
                        ...
                        ${isSelected ? 'bg-purple-500/40 text-purple-200' : 'bg-purple-500/20 text-purple-400'}`}>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-white/40 text-xs truncate">
                          {sup.specialization || 'General Supervisor'}
                        </p>
                        <div className="flex items-center gap-1.5 mt-0.5">
                          <StarDisplay rating={parseFloat(sup.average_rating || 0)} />
                          <span className="text-white/30 text-xs">
                            {sup.average_rating || 'No'} rating
                            {sup.total_reviews > 0 && ` (${sup.total_reviews})`}
                          </span>
                        </div>
                      </div>
                      <Shield size={14} className={isSelected ? 'text-purple-400' : 'text-white/20'} />
                    </button>
                  );
                })}
              </div>
          ...
          <div className="flex gap-3 pt-2">
            <button type="button"
              onClick={() => { setShowBook(false); reset(); setSelectedSupervisor(null); }}
        ...
        <Modal isOpen={!!viewAppt} onClose={() => setViewAppt(null)} title="Appointment Details" size="sm">
          <div className="space-y-3">
            {[
              ...
              { label: 'Supervisor', value: viewAppt.supervisor_name || 'Not chosen' },
