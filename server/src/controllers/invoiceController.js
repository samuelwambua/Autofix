...
const createInvoice = async (req, res) => {
  try {
    ...
    if (job.status !== 'completed')
      return res.status(400).json({ success: false, message: 'An invoice can only be created for a completed job.' });

    const invoiceExists = await pool.query('SELECT id FROM invoices WHERE job_id = $1', [job_id]);
    if (invoiceExists.rows.length > 0)
      return res.status(400).json({ success: false, message: 'An invoice already exists for this job.' });

    const partsResult = await pool.query(
      `SELECT COALESCE(SUM(quantity_used * unit_price), 0) AS parts_cost FROM job_parts WHERE job_id = $1`,
      [job_id]
    );
    ...
    const result = await pool.query(
      `INSERT INTO invoices (job_id, client_id, labour_cost, parts_cost, total_amount, notes, garage_id)
       VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *`,
      [job_id, job.client_id, labour_cost, parts_cost, total_amount, notes || null, req.garage_id]
    );
    return res.status(201).json({ success: true, message: 'Invoice created successfully.', data: result.rows[0] });
  } catch (error) {
...
const getAllInvoices = async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT i.*, c.first_name || ' ' || c.last_name AS client_name, c.phone AS client_phone,
       ...
       JOIN vehicles v ON jc.vehicle_id = v.id
       WHERE i.garage_id = $1 ORDER BY i.issued_at DESC`,
      [req.garage_id]
    );
...
const getMyInvoices = async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT i.*, v.make || ' ' || v.model AS vehicle_name, v.plate_number,
              jc.description AS job_description
       FROM invoices i
       JOIN job_cards jc ON i.job_id = jc.id
       JOIN vehicles v ON jc.vehicle_id = v.id
       WHERE i.client_id = $1 ORDER BY i.issued_at DESC`,
      [req.user.id]
    );
    return res.status(200).json({ success: true, count: result.rows.length, data: result.rows });
  } catch (error) {
  ...
  try {
    ...
    const result = await pool.query(
      `SELECT i.*, c.first_name || ' ' || c.last_name AS client_name,
              ...
              v.make || ' ' || v.model AS vehicle_name, v.plate_number, v.year,
              jc.description AS job_description, jc.actual_completion,
    ...
    if (req.garage_id && result.rows[0].garage_id !== req.garage_id)
      return res.status(403).json({ success: false, message: 'Access denied. Invoice not in your garage.' });

    const partsResult = await pool.query(
      `SELECT jp.quantity_used, jp.unit_price, jp.quantity_used * jp.unit_price AS subtotal,
              i.name AS part_name, i.sku
       FROM job_parts jp JOIN inventory i ON jp.part_id = i.id
       WHERE jp.job_id = $1`,
      [result.rows[0].job_id]
    );
    return res.status(200).json({ success: true, data: { ...result.rows[0], parts_breakdown: partsResult.rows } });
  } catch (error) {
...
const recordPayment = async (req, res) => {
  try {
    ...
    if (!payment_method || !amount_paid)
      return res.status(400).json({ success: false, message: 'Please provide payment method and amount paid.' });

    if (!validMethods.includes(payment_method))
      return res.status(400).json({ success: false, message: `Invalid payment method.` });

    const invoiceExists = await pool.query(
      'SELECT * FROM invoices WHERE id = $1 AND garage_id = $2', [id, req.garage_id]
    );
    if (invoiceExists.rows.length === 0)
      return res.status(404).json({ success: false, message: 'Invoice not found.' });
    ...
    if (paid_amount <= 0)
      return res.status(400).json({ success: false, message: 'Amount paid must be greater than 0.' });

    const status  = paid_amount >= total ? 'paid' : 'partially_paid';
    const paid_at = status === 'paid' ? new Date() : null;

    const result = await pool.query(
      `UPDATE invoices SET payment_method = $1, status = $2, paid_at = $3, updated_at = NOW()
    ...
    return res.status(200).json({
      ...
      message: status === 'paid'
        ...
        : `Partial payment of KES ${paid_amount.toLocaleString()} recorded.`,
      data: result.rows[0],
    });
  } catch (error) {
...
const updateInvoice = async (req, res) => {
  try {
    ...
    if (labour_cost !== undefined) {
      ...
      total_amount    = new_labour_cost + parseFloat(invoiceExists.rows[0].parts_cost);
    }

    const result = await pool.query(
      `UPDATE invoices SET labour_cost = $1, total_amount = $2,
         notes = COALESCE($3, notes), updated_at = NOW()
       WHERE id = $4 RETURNING *`,
      [new_labour_cost, total_amount, notes, id]
    );
    return res.status(200).json({ success: true, message: 'Invoice updated successfully.', data: result.rows[0] });
  } catch (error) {
  ...
  try {
    const summary = await pool.query(
      `SELECT COUNT(*) AS total_invoices,
       ...
       FROM invoices WHERE garage_id = $1`,
      [req.garage_id]
    );
...
module.exports = { createInvoice, getAllInvoices, getMyInvoices, getInvoiceById, recordPayment, updateInvoice, getBillingSummary, deleteInvoice };